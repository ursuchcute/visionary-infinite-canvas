...
+ [调整] 精简画布、提示词和资产页面顶部信息，隐藏独立配置页及导航栏外观；顶部左侧仅保留主页图标，三个业务入口依次移至右侧配置按钮之前，并改为橙色渐变滑块平滑切换的分段选择器。
...
+ [优化] 左侧面板开关移至页面垂直中点并改为贴边直角的单线左右箭头，底部主工具栏新增上下展开和收起控制。
+ [优化] 将底部工具栏收起态调整为更宽、更扁的按钮。
+ [优化] 画布右上角功能入口改为带文字的插件、文档、配置和快捷键，并用可折叠边框组统一收纳。
