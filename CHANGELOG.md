...
+ [调整] 精简公共顶部入口：左侧仅保留主页图标，右侧仅显示画布、资产、配置和主题，隐藏提示词与 GitHub 入口，业务入口使用橙色渐变滑块平滑切换。
...
+ [优化] 「我的画布」采用 Visionary 深黑背景、左对齐无边框文字操作和大圆角作品卡片，移除无画布时的引导模块，并保留浅色主题适配。
+ [调整] 「我的资产」隐藏关键词搜索模块，保留类型筛选及导入、导出和新增操作。
...
+ [优化] 节点提示词输入框下方的模型、生成设置和发送操作改为一体化扁平工具栏，并以分隔线保持层级。
