...
const hostContracts = readWebSource("src/services/api/visionary-host/contracts.ts");
...
assert.match(hostContracts, /clientReleaseVersion\?: string/);
assert.match(hostClient, /\.\.\.\(VISIONARY_RELEASE_VERSION \? \{ clientReleaseVersion: VISIONARY_RELEASE_VERSION \} : \{\}\)/);
...
assert.match(hostContracts, /imageDeliveryAck\?: boolean/);
assert.match(hostClient, /imageDeliveryAckEnabled = false/);
const imageRecoverySource = hostClient.slice(hostClient.indexOf("export async function recoverStoredVisionaryHostImages"), hostClient.indexOf("export async function requestVisionaryHostText"));
assert.equal((imageRecoverySource.match(/finalizeRecoveredVisionaryHostImage\(/g) || []).length, 4);
const imageDeliveryFinalizer = imageRecoverySource.slice(imageRecoverySource.indexOf("async function finalizeRecoveredVisionaryHostImage"));
assert.match(operations, /localDeliveryCompletedAt\?: number/);
assert.match(imageDeliveryFinalizer, /let locallyDelivered = Boolean\(record\.localDeliveryCompletedAt\)/);
assert.match(imageDeliveryFinalizer, /if \(!locallyDelivered\) \{[\s\S]*if \(!\(await onTerminal\(record\)\)\) return false/);
assert.match(imageDeliveryFinalizer, /updateHostOperation\(record\.clientOperationId, \{ localDeliveryCompletedAt \}\)/);
assert.match(imageDeliveryFinalizer, /record\.status === "completed" && imageDeliveryAckEnabled/);
assert.match(imageDeliveryFinalizer, /hostJson<VisionaryHostImageDeliveryAckResponse>\("\/images\/delivery-ack"/);
assert.match(imageDeliveryFinalizer, /clientReleaseVersion: VISIONARY_RELEASE_VERSION/);
assert.match(imageDeliveryFinalizer, /HOST_IMAGE_DELIVERY_ACK_TIMEOUT_MS/);
assert.match(imageDeliveryFinalizer, /withHostDeliveryAckTimeout\(signal,/);
assert.match(imageDeliveryFinalizer, /catch \(error\) \{\s*if \(signal\?\.aborted && isAbortError\(error\)\) throw error;\s*return false;/);
assert.ok(imageDeliveryFinalizer.indexOf("await onTerminal(record)") < imageDeliveryFinalizer.indexOf('hostJson<VisionaryHostImageDeliveryAckResponse>("/images/delivery-ack"'));
assert.ok(imageDeliveryFinalizer.indexOf('hostJson<VisionaryHostImageDeliveryAckResponse>("/images/delivery-ack"') < imageDeliveryFinalizer.indexOf("await acknowledgeHostOperation(record.clientOperationId)"));
assert.match(hostedConstants, /import\.meta\.env\.DEV \? "dev" : ""/);
assert.match(hostedDistAudit, /Hosted build is missing a non-empty release version/);
...
assert.doesNotMatch(textRequestSource, /delivery-ack|finalizeRecoveredVisionaryHostImage/);
...
assert.match(project, /state\.bootstrap\?\.features\.imageDeliveryAck === true/);
assert.match(project, /recoverStoredVisionaryHostImages\([\s\S]*imageDeliveryAckEnabled,/);
